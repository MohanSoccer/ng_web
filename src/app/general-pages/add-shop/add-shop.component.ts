...
export class AddShopComponent implements OnInit, AfterViewInit {
  ...
  deleteMedia(mediaType:string = null) {
    if(mediaType == "banner")
    {
     this.imageFormvalue = this.addShopForm.get('shop_banner_media_id').value;
    }
    if(mediaType == "gst")
    {
      this.imageFormvalue = this.addShopForm.get('owner').get("gst_info_id").value;
    }
    if(mediaType == "pan")
    {
      this.imageFormvalue = this.addShopForm.get('owner').get("pan_id").value;
    }

    if(mediaType != null) {
    this.utility.deleteImage(this.imageFormvalue).subscribe(
      (res: any) => {
        if(mediaType == "banner") {
          this.bannerUrl = undefined;
          this.bannerPath = undefined;
        }
        if(mediaType == "gst") {
          this.gstUrl = undefined;
          this.gstPath = undefined;
        }
        if(mediaType == "pan") {
          this.panUrl = undefined;
          this.panPath = undefined;
        }
